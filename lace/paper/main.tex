...
\newcommand{\lovelace}{LovelaceJEPA}
\newcommand{\sg}{\mathrm{sg}}
\newcommand{\Estud}{E_{\theta}}
\newcommand{\Efroz}{E_{\bar\theta}}

\title{\lovelace: Online Encoder Adaptation by\\
...
\begin{quote}\small\emph{``The Analytical Engine has no pretensions whatever
to originate anything. It can do whatever we know how to order it to
perform.''} --- Ada Lovelace, Note G (1843)
\end{quote}
...
by the very encoder being adapted. Nothing in the objective prefers ``model
...
calibration time. Running the published recipe on a deployed
...
its consumers were calibrated on. We call the resulting model \lovelace{},
because its adaptation ``has no pretensions to originate anything'': it may
only re-express the pretrained representation, never invent a new one.
...
The name is the method: adaptation that has ``no pretensions to originate
anything.'' The student may learn new dynamics at full learning rate, but the
meaning of the coordinates is inherited, not invented.
...
beat unlaced planning success in aggregate --- dynamics shifts: unlaced
$90.0\%$, laced-frozen $88.3\%$, frozen $78.3\%$; visual shifts:
laced-frozen $72.5\%$ vs unlaced $71.7\%$, frozen $66.7\%$; held-out
layouts: laced-ema $44.4\%$ vs unlaced $45.6\%$, frozen $35.6\%$. With the
gradient-descent planner on dynamics shifts the anchored arm is strongest
($90.0\%$ vs $70.0\%$ unlaced, $73.3\%$ frozen): GD planning is more
sensitive to the smoothness of the cost landscape, which the anchor
preserves. On PushObj-mini the planner operates near its noise floor
(Section~\ref{sec:setup}; success $10$--$25\%$ everywhere), and per-arm
differences are within error bars; we report the suite for completeness and
lean on the maze and deployment data for the planning claims.
...
\caption{Symmetric-LR sweep on the high-damping shift. At
$\eta_{\text{enc}}{=}10^{-3}$ the self-referential target collapses planning
($33.3\%$, below the frozen baseline) while the anchored target keeps
improving ($86.7\%$).}
\label{fig:e3}
\end{figure}
...
evaluating on unseen shapes, adaptation helps most exactly where data is
scarcest (unlaced $+$laced both lift $K{=}1,2$ success), consistent with
AdaJEPA's data-compensation result; anchored and self-referential targets
are again indistinguishable in success. Probe AUC in this regime is
dominated by the quality of the pretrained representation itself
(frozen-arm AUC falls from $0.96$ at $K{=}4$ to $0.67$ at $K{=}1$),
so head-consistency comparisons at $K{=}1$ mostly measure the probes'
failure to fit, not adaptation damage; we flag this as a limit of the probe
...
shapes: a fast anchor ($\tau{=}0.9$) is self-reference in slow motion ---
probe AUC falls to $0.68$ and planning success to $5\%$ --- while
$\tau{=}0.996$ and the frozen anchor hold AUC at $0.85$--$0.91$. Consistency
requires the anchor to move at most negligibly over the horizon on which
downstream consumers were calibrated; when in doubt, freeze it.
...
\url{https://github.com/bourkefloyd/lovelacejepa}. The
...
and Ada Lovelace for the epigraph and the acronym.\footnote{\lace{}: Learning
Anchored Consistent Embeddings. The model is \lovelace{} because it is an
Ada-JEPA that refuses to originate anything.}
...
\label{app:env}
% Shape cell definitions, physics constants, data-gen parameters,
% oracle-solvability calibration table.

\section{Probe training details}
\label{app:probes}
% Pair sampling, architectures, fit quality on pretrained latents.

\section{Full result tables}
\label{app:tables}
% Per-shift tables with all arms, seeds, and gate verdicts (pass/fail).
